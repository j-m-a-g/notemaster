...
class FileViewing {
   ...
   */
  static tasksOnceFileOpen(unhiddenView, fileInputObject) {
  ...
  static closeCurrentFile() {
    ...
    webpageView.removeAttribute("src");
  ...
  static readHTMLNote() {
    ...
    fileReader.readAsText(event.target.files[0]);
    this.tasksOnceFileOpen("anotherNoteView", "anotherNoteFileInput");
  }
  ...
  static viewCodeFile() {
    ...
    this.tasksOnceFileOpen("codeFileView", "codeFileInput");
  }
   ...
   */
  static checkURLInput(URLInputObject) {
    if (document.querySelector(`#${URLInputObject}`).value === "") {
      NoteMaster.displaySnackbar("Please enter a valid URL");
      return false;
    } else {
      return true;
    }
   ...
   */
  static parseCloudDocumentURL() {
    // Checks whether the inputted URL is correct
    if (this.checkURLInput("URLToCloudFile")) {
      ...
      if (onedriveOrigin.checked) {
        ...
        NoteMaster.toggleDialog(false, "insertCloudURLDialog", null);
      ...
      NoteMaster.addHistoryEntry("URLToCloudFile");
      NoteMaster.toggleViewer(true, "cloudFileViewer");
      this.tasksOnceFileOpen("cloudFileView", null);
    }
  }
   ...
   */
  static verifyIfiFrameInEmbed() {
    if (embeddedCode.value.includes("</iframe>")) {
      customEmbedViewer.innerHTML = embeddedCode.value;
      noFileSelected.hidden = true;
      viewerStatusBar.hidden = true;
      NoteMaster.toggleDialog(false, "customEmbed", null);
      NoteMaster.toggleViewer(true, "customEmbedViewer");
      NoteMaster.dynamicallySetHeight();
      this.tasksOnceFileOpen(null, null);
    } else {
      NoteMaster.throwAppError(
